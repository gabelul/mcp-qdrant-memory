...
async function runTests() {
  ...
  await suite.test('scrollAll handles empty collection gracefully', async () => {
    ...
    suite.assert(Array.isArray(result.entities), 'Empty collection should return empty entities array');
    suite.assert(Array.isArray(result.relations), 'Empty collection should return empty relations array');
  });
  ...
  await suite.test('scrollAll retrieves actual data correctly', async () => {
    ...
    console.log(`   Found ${result.entities.length} entities, ${result.relations.length} relations`);
    ...
    if (result.relations.length > 0) {
      ...
      suite.assert(typeof relation.relationType === 'string', 'Relation should have relationType string');
      console.log(`   Sample relation: ${relation.from} -> ${relation.to} (${relation.relationType})`);
    }
  });
  ...
  await suite.test('KnowledgeGraphManager.getGraph parameter handling', async () => {
    ...
    suite.assert(Array.isArray(qdrantResult.relations), 'Qdrant mode should return relations array');
    
    console.log(`   Qdrant mode retrieved ${qdrantResult.entities.length} entities, ${qdrantResult.relations.length} relations`);
  });

  // Test 6: Error handling and fallback behavior
  await suite.test('Error handling and fallback behavior', async () => {
    const mockManager = {
      graph: { entities: [{ name: 'fallback', entityType: 'test', observations: [] }], relations: [] },
      qdrant: {
        async scrollAll() {
  ...
  await suite.test('Performance test - scrollAll pagination', async () => {
    ...
    console.log(`   Retrieved ${result.entities.length + result.relations.length} items in ${duration}ms`);
  ...
  await suite.test('Data consistency - entities and relations structure', async () => {
    ...
    if (result.entities.length > 0 && result.relations.length > 0) {
      const validRelations = result.relations.filter(r => 
        entityNames.includes(r.from) && entityNames.includes(r.to)
      );
      console.log(`   ${result.relations.length} relations, ${validRelations.length} with valid entity references`);
    }
  });
  ...
  await suite.test('Offset pagination logic validation', async () => {
    ...
    console.log(`   Pagination completed successfully with ${result.entities.length + result.relations.length} total items`);
  });
